...
Puis, avec l'instruction JavaScript `filter`, on crée un tableau contenant uniquement l'année sélectionnée et on effectue une jointure entre les données et le fond de carte grâce à la bibliothèque `bertin` (*on peut aussi télécharger la bibliothèque [ici](https://cdn.jsdelivr.net/npm/bertin@1.6.3)*).
...
Vous pouvez faire varier les paramètres `speed` et `step` pour changer la vitesse (qui dépend des capacités de votre ordinateur). Vous pouvez aussi appliquer cette fonction à la *Rotation (x)* en utilisant `interval: [-90, 90]`
