...
- voir ce que font les autres, faire de la veille, Suivre les compte de son choix (comme sur une réseau social)
- échanger avec la **<ins>communauté</ins>** (commentaires, forum, ...)
...
Si on ne souhaite pas créer de compte sur observablehq, il est possible de faire du Observable sans Observable, en travaillant avec le logiciel Quarto. En effet, **<ins>le runtime d'Observable est Open Source</ins>** et peut donc être utilisé dans d'autres contextes et dans d'autres logiciels. Cela permet aussi de travailler en local sur son ordinateur sans connexion internet.
...
On charge la bibliothèque avec l'instruction `require` en indiquant le numéro de version.
...
Lien : [https://cdn.jsdelivr.net/npm/bertin@1.6.1](https://cdn.jsdelivr.net/npm/bertin@1.6.1)
...
Les styles (couleurs, transparence, épaisseur, etc) reprénnent les nom des attributs SVG. 
...
Il existe de nombreuses couches de mise en page : `header`, `footer`, `graticule`, `outline`, etc. Elles ont des paramètres par défaut.

```{ojs}
bertin.draw({ 
  layers: [
    { geojson: world2019, fill: "#e971eb" },
    {type : "graticule"},
    {type: "outline"},
    {type: "header", text: "Le Monde est beau"},
    {type: "footer", text: "Source de la carte"}
  ]
})
```
...
```{ojs}
viewof margin = Inputs.range([0, 500], {value: 0, step: 1})
// La carte
bertin.draw({ 
  params: {width: 1000, margin: margin, background: "#CCC"},
  layers: [
    { geojson: world2019, fill: "#e971eb" },
    {type : "graticule"},
    {type: "outline"},
...
En cartographie thématique, il est souvent utile de généraliser (simplifier) un fond de carte. Cela permet d'améliorer la lecture et d'alleger le fond de carte à afficher. Une façon de faire cela est d'utiliser la bibliothèque `geotoolbox`
...
Puis, on affiche la carte

```{ojs}
bertin.draw({ layers: [ { geojson: world2019_simpl, fill: "#e971eb" }]})
```
...
bertin.draw({ 
  ...
  layers: [ 
    { type: "tissot", stroke: "none", fill:"#CCC", fillOpacity: 0.3},
    { type: "outline", stroke: "#e971eb", strokeWidth:2, fill: "none" },
    { geojson: world2019_simpl, fill: "#e971eb" },
    { type: "geolines", stroke: "#e971eb", strokeWidth:1, fill: "none" },
  ]
})
```
