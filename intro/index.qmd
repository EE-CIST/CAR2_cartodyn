...
-   Les performanbces s'émliorent encore avec WebGPU, qui permet de faire des calculs particulièrement efficaces pour l'execution d'algorithmes tres paralleles comme le traitement d'image, le machine learning et le rendu 3D. 
...
L'OJS a été créé par Mike Bostock, qui est aussi le developpeur de la bibliothèue **D3.js** (ou D3). D3 a été dircetement intégrée à l'Observable JavaScript. 
...
- D3 est populaire (200 millions de téléchargements et 100 000 étoiles)
- D3 est flexible (approche bas niveau qui permetd e tout faire)
...
Mais c'est le langage de base pour faire de la visualisation de données pour le web. La librairie `Plot` (que nous venons de voir) et la librairie `bertin` (que nous allons voir) sont tous deux construits avec D3.
...
import {africa} from "@neocartocnrs/bertin-js-mashroom"
```
