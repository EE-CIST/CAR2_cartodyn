...
[bertin](https://github.com/neocarto/bertin) est une bibliothèque (un ensemble de fonctionnalités) écrite en [JavaScript](https://developer.mozilla.org/fr/docs/Web/JavaScript) qui permet de réaliser des cartes thématiques pour le web. Son développement s'appuie en grande partie sur le librairie javascript [d3.js](https://d3js.org/) développée par Mike Bostock depuis 10 ans. Le développement a débuté en novembre 2021. Il y a 8 contrubuteurs. 240 ⭐ sur Github.
...
Lien de téléchargement : [https://cdn.jsdelivr.net/npm/bertin\@1.6.2](https://cdn.jsdelivr.net/npm/bertin@1.6.1)
...
En cartographie thématique, il est souvent utile de généraliser (simplifier) un fond de carte. Cela permet d'**améliorer la lecture** et de **réduire le poids** le fond de carte à afficher, ce qui ets très important en cartographie sur le web. Une façon de faire cela est d'utiliser la bibliothèque `geotoolbox`

On charge la bibiothèque avec l'instruction `require`

```{ojs}
geo = require("geotoolbox@1.9.3")
```

*(On peut aussi la télécharger ici : [https://cdn.jsdelivr.net/npm/geotoolbox\@1.9.3](https://cdn.jsdelivr.net/npm/geotoolbox@1.9.3))*
...
De nombreuses projection sont disponibles dans l'écosystème de D3.js grace aux modules `d3.geo` et `d3.geoprojection`: `"Polar"`,`"Spilhaus"`,`"InterruptedSinusoidal"`, `"Armadillo"`, `"Baker"`, `"Gingery"`, `"Berghaus"`,`"Loximuthal"`, `"Healpix"`, `"InterruptedMollweideHemispheres"`, `"Miller"`, `"Aitoff"`, etc.
...
Bien sûr, avant de réaliser une carte choroplèthe, il est toujours nécéssaire de caractériser la distribution statistique. Poutr cela, on peut utiliser la bibliothèque `Plot`. Il y a plein de façons de faire.
...
Astuce : On peut prendre 2 variables de stock pour calculer directeme,nt le ratio. Ici, PIB par habitant.
...
Le résultat ressemble à des "points Bertin" qu'on peut paramétrer. <br/>
...
//| echo: false
//| eval: true
viewof stepr = Inputs.range([5, 30], {value: 8, step: 1, label: "Step"})
viewof blur = Inputs.range([0, 2], { label: "Blur", step: 0.01, value: 0.4 })
viewof hauteur = Inputs.range([30, 100], { label: "Hauteur", step: 5, value: 100 })
bertin.draw({ 
params: {margin: 20},
layers: [ 
...
<br/>Apprendre à réaliser des cartes interactives<br/>avec Quarto et la bibliothèque JavaScript `bertin` <br/>
