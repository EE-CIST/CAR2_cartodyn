...
Dans ce TP, nous privilégions l'utilisation du logiciel [Rstudio](https://posit.co/download/rstudio-desktop/).
...
-   Cliquez sur "Create Empty Document".
-   Mettez votre nom
...
Pour chaque *chunck*, vous pouvez définir avec `echo` si vous souhaitez que le code s'affiche ou non dans votre notebook final. Avec `eval`, vous choisissez si le code doit s'executer ou non.
...
Pour générer le document, il faut clicher sur le bouton `Render` ou tiliser le racourci clavier *Ctrl+Shift+K*.
...
*Nb : pour travailler offline, vous pouvez aussi télécharger la librairie bertin [ici](https://cdn.jsdelivr.net/npm/bertin@1.6) et indiquer son emplacement. Par exemple si vous le mettez dans le repertoire lib : *
...
La fonction `quickdraw()` <a href="https://observablehq.com/@neocartocnrs/bertin-js-quickdraw-table2geo"><img src="img/logo_exemple.png"/></a> permet de visualiser rapidement n'importe quel fond de carte. Le second paramètre permet de définir la largeur de la carte (en pixels). La hauteur est déduite automatiquement. 
...
Il y a plusieurs façons de cartographier des données quantitatives relatives. Sur écran, en cartographie numérique, on choisra la plupart du temps de réaliser une carte choroplèthe avec des couleurs ordonnées. Un étape de discrétisation préalable est nécéssaire. Elle nécessite d'étudier la forme de la distribution statistique. Pour celà, on utilisera la librairie `Plot` <a href="https://github.com/observablehq/plot"><img src="img/logo_doc.png"/></a> <a href="https://observablehq.com/collection/@observablehq/plot"><img src="img/logo_exemple.png"/></a> disponible nativement dans les celles `{ojs}`.

Il y a plusieurs façons de visualister la forme d'une distribution <a href="https://observablehq.com/@cartographie/les-discretisations?collection=@cartographie/cours"><img src="img/logo_exemple.png"/></a> Ici, on choisira un graphique avec des points et une transformation de type dodge.
...
# 3. Faire une carte interactive (1/3)

Avec Observable JavaScript, nous sommes dans un écosystème pleinement réactif 🔥 Cela signifie qu'il est possible de proposer à l'utilisateur des possibilités d'interaction avec la carte. Reprenons le cas de de la carte par symboles proportionnels... et personnalisons le un peu en remplaçant les valeurs écrites *en dur* par des valeurs issues des `Inputs` <a href="https://github.com/observablehq/inputs"><img src="img/logo_doc.png"/></a> <a href="https://observablehq.com/@observablehq/inputs"><img src="img/logo_exemple.png"/></a>

Il est également possible d'ajouter des infobulles <a href="https://observablehq.com/@neocartocnrs/bertin-js-tooltips?collection=@neocartocnrs/bertin"><img src="img/logo_exemple.png"/></a>
...
# Faire une carte interactive (2/3)

Il peut être souvent interessant de demander à l'utilisateur de choisir luin même l'indicateur qu'il souhaite cartographie. Pour cela, on peut le proposer dans une liste déroulante <a href="https://observablehq.com/@observablehq/input-select"><img src="img/logo_exemple.png"/></a>. Par exemple, on peut mettre dans un `array` , les indicateurs qualitatifs issus du CEPII :
...
Si on veut les labels dans la liste déroule, c'est un petit plus compliqué. Un façon de faire est de créer un `array` contenant à la fois les noms de variable et les labels. 

```{ojs}
indicators2 = [["Pays enclavé", "LOCKED"],["Colonisation par la France","COLFRA"], ["Colonisation par le Royaume-Uni","COLGBR"], ["Langue officielle française","LANGFR"],["Langue officielle anglaise","LANGEN"]]
```

Puis, on utilise utilise cet `array` sous forme de `Map` JavasCript.
...
Dans `bertin`, il ne reste plus qu'à utiliser le contenu de la site comme nom de variable.
...
# Faire une carte interactive (3/3)

Ici, on ne souhaite plus voir "flotter" l'Afrique. Donc, on importe un nouveau fichier contenant les pays du Monde. Cette couche servira à l'habillage. Dans les paramètres généraux de la carte, il faudra bien penser à définir l'`extent` pour que la carte ne s'affiche pas à l'échelle mondiale.
...
Puis, pour faire varier la carte précédente, on choisi de représenter les données avec le type `dotcartogram` qui décompose chaque cercke en un nombre de points de même valeurs, placés sur le centroid <a href="https://github.com/neocarto/bertin#dot-cartogram"><img src="img/logo_doc.png"/></a><a href="https://observablehq.com/@neocartocnrs/bertin-js-dots-cartograms?collection=@neocartocnrs/bertin"><img src="img/logo_exemple.png"/></a>. Le reste s'écrit grosso modo comme la carte précédente.
